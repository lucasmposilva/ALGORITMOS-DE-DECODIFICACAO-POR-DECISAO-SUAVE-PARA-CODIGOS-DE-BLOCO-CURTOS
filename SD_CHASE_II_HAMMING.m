function SD_CHASE_II_HAMMING(radius, vEbNo, wethr, output_method)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if(sumRef>=sumAlt) && (size(ccHD,1)==N)
            ...
            for x = 1:nCand
                ...
                temp(indexes) = tps(:,x);
                %
                [decDataTP, a] = myDec(temp,h,s,N,K);
                % if a correction is impossible
                if (a ~= -1)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% saving results
saveResults(output_method,results,header,["SD_CHASE_II_HAMMING",mat2str(vEbNo)]);